let choice = {};
const medicare = document.getElementById("medicare"); // elements to display tax information
const socialSecurity = document.getElementById("SS"); // elements to display tax information
const federalTax = document.getElementById("federal"); // elements to display tax information
const stateTax = document.getElementById("state"); // elements to display tax information
const careerInfo = document.getElementById("career"); // element to display career information
...
const ids = ['housing', 'essentials', 'student-loans', 'life-style', 'future-proofing'];

function getValues() {
  return ids.map(id => Math.max(0, parseFloat(document.getElementById(id).value) || 0));// ensures non-negative numbers and handles empty inputs as 0
}

const chart = new Chart(document.getElementById('pie'), { // creates the pie chart
  type: 'pie',
  data: { //  data for the chart
    labels,
    datasets: [{ // only one dataset for the pie chart
      data: getValues(),
      ...
      tooltip: {
        callbacks: {
          label: (ctx) => {
            ...
            return ` ${ctx.label}: ${ctx.raw.toLocaleString()} (${pct}%)`; // formats the tooltip to show value and percentage
          }
        }
      }
    }
...
function buildLegend(vals) { // builds a custom legend with percentages
  const total = vals.reduce((a, b) => a + b, 0); // calculates the total for percentage calculations
  const el = document.getElementById('legend'); // gets the legend container
  el.innerHTML = labels.map((l, i) => { // creates legend items with color boxes and percentages
    const pct = total > 0 ? ((vals[i] / total) * 100).toFixed(1) : '0.0'; // calculates percentage for each category
